...
import { getConsent, onConsentChange, setConsent } from '@/lib/analytics/consent'
import { isGaConfigured } from '@/lib/analytics/gtag'

const subscribe = (onStoreChange: () => void) => onConsentChange(() => onStoreChange())
const getSnapshot = () => isGaConfigured() && getConsent() === null
 ...
 * всегда `false` (SSR не видит localStorage). Клик сам скрывает баннер без
 * отдельного `setVisible` — `setConsent` синхронно шлёт событие, которое
 * `onConsentChange` пробрасывает в `onStoreChange`, и `getSnapshot` тут же
 * пересчитывается в `false` (согласие больше не `null`).
 */
